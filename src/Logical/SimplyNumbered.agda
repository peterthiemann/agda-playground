...
data _⟶_ : Expr zero → Expr zero → Set where
  ...
  β₁ : ∀ {s}{w}
    ...
    → (abs₁ : ALL AbsValue s)
    → Value w
    → app s w ⟶ mapE (λ v → mapE (sub₁ v) (foldALL absbody abs₁)) w
...
atomic : Expr zero → Set
atomic e = Value e × NonEmpty e × NonConcat e

Atom : Set
Atom = Σ (Expr zero) atomic
...
-- ** value interpretation of numbered types
...
𝓥-≢ε : ∀ {e}{μ} → e ∈ 𝓥⟦ μ ⟧ → NonEmpty e
𝓥-≢ε e∈ = 𝓥-atomic e∈ .proj₂ .proj₁

𝓥-≢· : ∀ {e}{μ} → e ∈ 𝓥⟦ μ ⟧ → NonConcat e
𝓥-≢· e∈ = 𝓥-atomic e∈ .proj₂ .proj₂

𝓥-value : ∀ {e}{μ} → e ∈ 𝓥⟦ μ ⟧ → Value e
𝓥-value e∈ = 𝓥-atomic e∈ .proj₁
...
toAbs : ∀ {μ₁ η₂ μ₂}{x} → 𝓥⟦ μ₁ ⇒ ⟨ η₂ , μ₂ ⟩ ⟧ x → AbsValue x
toAbs (μ₀ , b , refl , _ , _) = v-abs μ₀ b
...
atomic-𝓥-value (AP e∈𝓥) e≢ε e≢· = 𝓥-value e∈𝓥

all-𝓥-nf→value : ∀ {e}{μ} → ALL 𝓥⟦ μ ⟧ e → monoidal-nf e → Value e
all-𝓥-nf→value Aε nf = vε
all-𝓥-nf→value (AP e∈𝓥) nf = 𝓥-value e∈𝓥
...
value-𝓦 {ημ = ⟨ η , μ ⟩} (all∈𝓥 , nf , len∈) = value-all-nf all∈𝓥 nf
  where
    ...
    atomic-from-all (AP e∈𝓥) e≢ε e≢· = 𝓥-value e∈𝓥
    ...
    value-all-nf (AP e∈𝓥) nf = 𝓥-value e∈𝓥
...
atomic-length : (v : Expr zero) → NonEmpty v → NonConcat v → lengthE v ≡ 1
...
... | w₁w₂ , (all₁₂ , mono₁₂ , len₁₂) , red
  = (v · w₁w₂) , ((ap₁ A· all₁₂) , (v≢ε , (·-ALL-preserves-≢ε' w₁ w₂ {w₁w₂} (mapALL 𝓥-value all₁) (mapALL 𝓥-value all₂) mono₁ mono₂ (λ ()) red  , (v≢· , mono₁₂))) , subst (λ □ → (□ +ℕ lengthE w₁w₂) ∈∈ 𝓝⟦ ADD η₁ _ ⟧) (sym (atomic-length v v≢ε v≢·)) (ADD-DEC (suc-not-empty {η₁} len₁) len₁₂)) , ⟶-step mon-·-assoc (ξ-tail-* (𝓥-value (atomic-ALL ap₁ v≢ε v≢·)) red)
compatible-· {w₁ = v · w₁} {w₂ = w₂} {η₁} ((ap₁ A· all₁) , (v≢ε , w₁≢ε , v≢· , mono₁) , len₁) w₂∈@(all₂@(AP {e≢ε = w₂≢ε} {e≢· = w₂≢·} x) , mono₂ , len₂)
  rewrite atomic-length v v≢ε v≢·
  with compatible-· {w₁ = w₁} {w₂ = w₂} {η₁ = DEC η₁} (all₁ , mono₁ , DEC-sound {η₁} len₁) w₂∈
... | w₁w₂ , (all₁₂ , mono₁₂ , len₁₂) , red
  = (v · w₁w₂) , ((ap₁ A· all₁₂) , (v≢ε , (·-ALL-preserves-≢ε' w₁ w₂ {w₁w₂} (mapALL 𝓥-value all₁) (mapALL 𝓥-value all₂) mono₁ mono₂ w₂≢ε red  , (v≢· , mono₁₂))) , subst (λ □ → (□ +ℕ lengthE w₁w₂) ∈∈ 𝓝⟦ ADD η₁ _ ⟧) (sym (atomic-length v v≢ε v≢·)) (ADD-DEC (suc-not-empty {η₁} len₁) len₁₂)) , ⟶-step mon-·-assoc (ξ-tail-* (𝓥-value (atomic-ALL ap₁ v≢ε v≢·)) red)
...
-- compatibility lemmas / one corresponding to each typing rule
...
compatible-abs {μ = μ} {s = s} {ημ = ημ} sem-s σ σ∈
  = sub σ (abs μ s) , ((AP-abs (μ , (sub (liftSub σ) s , refl , <:ₜ-refl , λ v v∈𝓥 → subst (𝓔⟦ ημ ⟧) (sub-ext-lift {σ = σ} {v = v} {e = s}) (sem-s (extSub σ v) (ext-𝓖 σ∈ ((AP-𝓥 v∈𝓥) , value-monoidal-nf (𝓥-value v∈𝓥) , ≤-reflexive (sym (length-𝓥 v∈𝓥)) , ≤-reflexive (length-𝓥 v∈𝓥))))))) , tt , (s≤s z≤n , s≤s z≤n)) , ⟶-refl
...
compatible-app-s : ∀ {s₁}{s₂}{η₁ μ₁ η₂ μ₂ η₃ η η′}
  ...
  → MUL η₁ η₂ η′ → MUL η′ η₃ η
  → Γ ⊨ app s₁ s₂ ⦂ ⟨ η , μ₂ ⟩
compatible-app-s {η₁ = η₁}{μ₁ = μ₁}{η₂ = η₂}{μ₂ = μ₂}{η₃}{η}{η′} sem-s₁ sem-s₂ m m₁ σ σ∈
  with sem-s₁ σ σ∈
... | s , 𝓦-s@(all∈μ₁⇒η₂μ₂ , mono-s , len∈η₁) , sub-σ-s₁⟶*s
  with sem-s₂ σ σ∈
... | w , 𝓦-w@(all∈μ₁ , mono-w , len∈η₃) , sub-σ-s₂⟶*w
  using value-s ← value-𝓦 {ημ = ⟨ η₁ , μ₁ ⇒ ⟨ η₂ , μ₂ ⟩ ⟩} 𝓦-s
  using value-w ← value-𝓦 𝓦-w
  using abs-s ← mapALL toAbs all∈μ₁⇒η₂μ₂
  using reduce-to-redex ← (⟶*-trans (ξ-app₁-* sub-σ-s₁⟶*s) (ξ-app₂-* value-s sub-σ-s₂⟶*w))
  using reduce-redex ←  ⟶*-snoc reduce-to-redex  (β₁ value-s abs-s value-w)
  = let sem = app-s-seq-sem all∈μ₁ mono-w len∈η₃ m₁ in
  ...
  where
    ...
    app-s-single-sem
      ...
      → ∀ {v}
      → v ∈ 𝓥⟦ μ₁ ⟧
      → ∃[ w* ] w* ∈ 𝓦⟦ ⟨ η′ , μ₂ ⟩ ⟧ × (mapE (sub₁ v) (foldALL absbody (mapALL toAbs all∈)) ⟶* w*)
    app-s-single-sem Aε mono len m v∈
      = ε
      , (Aε , tt , <:₀-subset (MUL-left-empty (numOfLen-sub len) m) (z≤n , z≤n))
      , ⟶-refl
    app-s-single-sem {η₁ = η₁′} {η′ = η′} (AP {e = e} {e≢ε = e≢ε} {e≢· = e≢·} (μ₀ , b , refl , μ₁<:μ₀ , fvw)) mono len m {v} v∈
      rewrite mapE-sub₁ {w = v} {e = b}
      = <:ₙ-subset-𝓔 (<:ₙ-comb η₂<:η′ <:ₜ-refl) (fvw v v∈)
      where
        len! : 1 ∈∈ 𝓝⟦ η₁′ ⟧
        ...
        η₂<:η′ : η₂ <:₀ η′
        η₂<:η′ = MUL-left-one-super !<:η₁ m
    app-s-single-sem {s = s₁ · s₂} {η₁ = η₁′} {η′ = η′} (all₁ A· all₂) mono-s@(s₁≢ε , s₂≢ε , s₁≢· , mono₂) len m {v} v∈
      with all₁
    ... | Aε = ⊥-elim (s₁≢ε refl)
    ... | (_ A· _) = ⊥-elim (s₁≢· refl)
    ... | AP {e = e₁} {e≢ε = e₁≢ε} {e≢· = e₁≢·} (μ₀ , b , refl , μ₁<:μ₀ , fvw)
      rewrite mapE-sub₁ {w = v} {e = b}
      with app-s-single-sem all₂ mono₂ (<:₀-subset (concat-plus-super mono-s len) (mono-nonempty-len-plus mono₂ s₂≢ε)) m v∈
    ... | w₂* , w₂*∈𝓦 , red₂
      with <:ₙ-subset-𝓔 (<:ₙ-comb (MUL-left-one-super (<:₀-trans <:₀-!+ (concat-plus-super mono-s len)) m) <:ₜ-refl) (fvw v v∈)
    ... | w₁* , w₁*∈𝓦 , red₁
    ...
    app-s-seq-sem
      ...
      → lengthE w ∈∈ 𝓝⟦ η₃ ⟧
      → MUL η′ η₃ η
      → ∃[ w* ] w* ∈ 𝓦⟦ ⟨ η , μ₂ ⟩ ⟧ × (mapE (λ v → mapE (sub₁ v) (foldALL absbody (mapALL toAbs all∈μ₁⇒η₂μ₂))) w ⟶* w*)
    app-s-seq-sem Aε mono len m₁
      = ε
      , (Aε , tt , <:₀-subset (MUL-right-empty (numOfLen-sub len) m₁) (z≤n , z≤n))
      , ⟶-refl
    app-s-seq-sem {η₃ = η₃′} {η = η} (AP {e = e} {e≢ε = e≢ε} {e≢· = e≢·} v∈) mono len m₁
      with app-s-single-sem all∈μ₁⇒η₂μ₂ mono-s len∈η₁ m v∈
    ... | w* , w*∈𝓦η′ , red
      = <:ₙ-subset-𝓔 (<:ₙ-comb η′<:η <:ₜ-refl)
          (w* , w*∈𝓦η′
          , subst (λ z → z ⟶* w*) (sym (mapE-atomic e≢ε e≢·)) red)
      where
        len! : 1 ∈∈ 𝓝⟦ η₃′ ⟧
        len! = subst (λ n → n ∈∈ 𝓝⟦ η₃′ ⟧) (atomic-length e e≢ε e≢·) len
        ...
        η′<:η : η′ <:₀ η
        η′<:η = MUL-right-one-super !<:η₃ m₁
    app-s-seq-sem {w = w₁ · w₂} {η₃ = η₃′} {η = η} (all₁ A· all₂) mono-w@(w₁≢ε , w₂≢ε , w₁≢· , mono₂) len m₁
      with all₁
    ... | Aε = ⊥-elim (w₁≢ε refl)
    ... | (_ A· _) = ⊥-elim (w₁≢· refl)
    ... | AP {e = e₁} {e≢ε = e₁≢ε} {e≢· = e₁≢·} v₁∈
      with app-s-single-sem all∈μ₁⇒η₂μ₂ mono-s len∈η₁ m v₁∈
    ... | w₁*η′ , w₁*∈𝓦η′ , red₁′
      with app-s-seq-sem all₂ mono₂ (<:₀-subset (concat-plus-super mono-w len) (mono-nonempty-len-plus mono₂ w₂≢ε)) m₁
    ... | w₂* , w₂*∈𝓦η , red₂
      with <:ₙ-subset-𝓔 (<:ₙ-comb (MUL-right-one-super (<:₀-trans <:₀-!+ (concat-plus-super mono-w len)) m₁) <:ₜ-refl) (w₁*η′ , w₁*∈𝓦η′ , red₁′)
    ... | w₁* , w₁*∈𝓦η , red₁
      with compatible-· w₁*∈𝓦η w₂*∈𝓦η
    ... | w₁₂* , w₁₂*∈𝓦 , red₁₂
      = <:ₙ-subset-𝓔
          (<:ₙ-comb (ADD-self-super-plus +<:η₃ m₁) <:ₜ-refl)
          (w₁₂* , w₁₂*∈𝓦
          , ⟶*-trans
              (⟶*-trans
                (ξ-head-* (subst (λ z → z ⟶* w₁*) (sym (mapE-atomic e₁≢ε e₁≢·)) red₁))
                (ξ-tail-* (value-𝓦 w₁*∈𝓦η) red₂))
              red₁₂)
      where
        +<:η₃ : `+ <:₀ η₃′
        +<:η₃ = concat-plus-super mono-w len
