...
eval-soundness {S = mkQ 𝟙 T} ⊢𝓗 ⊢𝓢 (TVar {x = X s 𝟚} x∈) ⊨𝓔 ≤-q (EVarS (on-stack sacc) dec≡)
  = -, ≼-refl , genaccess-soundness-𝟚 ⊨𝓔 x∈ (on-stack sacc) _ dec≡ , ⊢𝓗 , ⊢𝓢
eval-soundness {S = mkQ 𝟚 T} ⊢𝓗 ⊢𝓢 (TVar {x = X s 𝟙} x∈) ⊨𝓔 ≤-q (EVarS (on-heap hacc) refl)
  = -, ≼-refl , genaccess-soundness ⊨𝓔 x∈ (on-heap hacc) , ⊢𝓗 , ⊢𝓢

----- abstraction

eval-soundness ⊢𝓗 ⊢𝓢 (TAbs {q = 𝟙} ⊢e qbdd {wf₁}{wf₂}) ⊨𝓔 ≤-refl EAbsH
  = -, ≼-refl , TVClos (restrict′ ⊨𝓔 qbdd) refl (is-bounded qbdd) ⊢e wf₁ wf₂ <⦂-refl , ⊢𝓗 , ⊢𝓢
-- why is the following case legal?
eval-soundness ⊢𝓗 ⊢𝓢 (TAbs {q = 𝟙} {S≤x = refl} ⊢e qbdd {wf₁}{≤-refl}) ⊨𝓔 ≤-bottop EAbsS
  = -, ≼-refl , TVClos (restrict′ ⊨𝓔 qbdd) refl (is-bounded qbdd) ⊢e wf₁ ≤-refl <⦂-refl , ⊢𝓗 , ⊢𝓢
...
-- q = 𝟚, ? → ? <⦂ 𝟙 → 𝟙
eval-soundness ⊢𝓗 ⊢𝓢 (TApp {S₁ = S₁@ (T₁ ^ 𝟙)} {S₂ = S₂} ⊢e ⊢e₁) ⊨𝓔 ≤-refl (EAppH {𝓢 = 𝓢}{𝓢′ = 𝓢′}{𝓢″ = 𝓢″} ⇓ ⇓₁ ⇓₂ refl)
...
... | Σₕₛ‴ , ≼Σ‴ , ⊢vres , ⊢𝓗‴ , ⊢𝓢‴
  = Σₕₛ‴ , ≼-trans ≼Σ′ (≼-trans ≼Σ″ ≼Σ‴) , <⦂-val-lift ⊢vres <⦂Sres , ⊢𝓗‴ , ⊢𝓢‴

-- q = 𝟚, ? → ? <⦂ 𝟚 → 𝟙
eval-soundness ⊢𝓗 ⊢𝓢 (TApp {S₁ = S₁@ (T₁ ^ 𝟙)} {S₂ = S₂} ⊢e ⊢e₁) ⊨𝓔 ≤-refl (EAppH {v₂ = v₂} {𝓢″ = 𝓢″} ⇓ ⇓₁ ⇓₂ refl)
  | Σₕₛ′ , ≼Σ′ , TVClos {q = 𝟚} {x = X s q₂} {S₁≤x = refl} ⊨𝓔′ 𝓢≡ qbd ⊢e₂ wf₁ wf₂ (SQual ≤-refl (SFun{S₃ = S₃}{S₁ = S₁′@ (T₁′ ^ 𝟚)}{S₂ = _}{S₄ = S₄@ (T₄ ^ 𝟙)} <⦂Sarg <⦂Sres)) , ⊢𝓗′ , ⊢𝓢′
  with eval-soundness ⊢𝓗′ ⊢𝓢′ ⊢e₁ (eval-preservation ⊢e (⊨-extend-Σ ≼Σ′ ⊨𝓔) ⇓) (q-of-mono <⦂Sarg) ⇓₁
... | Σₕₛ″ , ≼Σ″ , ⊢varg , ⊢𝓗″ , ⊢𝓢″
  using ≼-ext ← ≼-extend-Σ 𝟚 S₁′
