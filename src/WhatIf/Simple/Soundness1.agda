module Simple.Soundness1 where
...
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos ∀⊨𝓔 𝓢≡ qbd ⊢e₂ wf₁ wf₂ (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁}{S₂ = S₂}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢}{q = 𝟚} {q₁ = 𝟙}{𝓢′ = 𝓢′}{𝓢″ = 𝓢″} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft  wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos {S₁≤x = refl} ∀⊨𝓔 𝓢≡ qbd ⊢e₂ wf₁ wf₂ (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁}{S₂ = S₂}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
                      (↑wf-𝓗 ih)
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢}{q = 𝟚} {q₁ = 𝟚}{𝓢′ = 𝓢′}{𝓢″ = 𝓢″} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft  wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos ∀⊨𝓔 𝓢≡ qbd ⊢e₂ wf₁ wf₂ (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁}{S₂ = S₂}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢}{q = 𝟙} {q₁ = 𝟚}{𝓢′ = 𝓢′}{𝓢″ = 𝓢″} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft  wf-𝓗 wf-𝓢 ≤-top ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁ ; ↑⊢v = TVClos ∀⊨𝓔 𝓢≡ qbd ⊢e₂ wf₁ wf₂ (SQual qsub (SFun {S₃ = S₃}{S₁ = S₁}{S₂ = S₂}{S₄ = S₄} S₁<⦂S₂ S₃<⦂S₄)) ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ ; ↑cls = ↑cls₁ ; ↑wf-𝓗 = ↑wf-𝓗₁ ; ↑wf-𝓢 = ↑wf-𝓢₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
...
... | ih@record { ↑Σₕ′ = Σₕ′ ; ↑Σ≼ₕ = Σ≼ₕ′ ; ↑Σ≼ₛ = Σ≼ₛ′
                ; ↑⊢v = (TVClos {x = X s _} {S₁≤x = S₁≤x} ∀⊨𝓔 refl qbd ⊢e₂ wf₁ ≤-refl (SQual qsub (SFun{S₁ = S₁@ (T₁ ^ 𝟙)} S₁<⦂S₂ S₃<⦂S₄)))
                ; ↑⊢𝓗 = _ ; ↑⊢𝓢 = _ ; ↑𝓢≼ = _ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
...
... | ih₂
  = record
      { ↑Σₕ′ = ↑Σₕ′ ih₂
      ; ↑Σₛ′ = Σₛ″
      ; ↑Σ≼ₕ = ≼-trans Σ≼ₕ′ (≼-trans Σ≼ₕ″ (↑Σ≼ₕ ih₂))
      ; ↑Σ≼ₛ = ≼-trans Σ≼ₛ′ Σ≼ₛ″
      ...
      ; ↑wf-𝓗 = wfl-ext-≼ₛ {!↑𝓢≼ ih₂!} (↑wf-𝓗 ih₂)
      ; ↑wf-𝓢 = ih₁ .↑wf-𝓢
      }

-- application, case 2: 𝟚-closure with 𝟙-argument
-- in this case, I need to know that the closure captures a substack (in the sense of ≼ₛ) of the current stack

eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl{-R-} (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢} {q = 𝟚} {q₁ = 𝟙} {𝓢′ = 𝓢′}{𝓢″ = 𝓢″} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = ↑Σ≼ₕ₁ ; ↑Σ≼ₛ = ↑Σ≼ₛ₁
             ; ↑⊢v = TVClos {𝓢 = 𝓢ᶜ} {x = X s _} {S₁≤x = S₁≤x} ∀⊨𝓔 refl qbd ⊢e₂ wf₁ wf₂ (SQual qsub (SFun{S₃ = S₃}{S₁ = S₁@ (T₁ ^ 𝟙)}{S₂ = S₂}{S₄ = S₄}{wf₄ = wf₄} S₃<⦂S₁ S₂<⦂S₄))
             ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = ↑𝓢≼₁ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-bottop (TApp ⊢e ⊢e₁) (EApp {𝓢 = 𝓢} {q = 𝟚} {q₁ = 𝟙} {𝓢′ = 𝓢′}{𝓢″ = 𝓢″}{𝓢‴ = 𝓢‴} q₂≤q ⇓ ⇓₁ ⇓₂ refl)
  with eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-refl ⊢e ⇓
... | ih@record { ↑Σₕ′ = ↑Σₕ′₁ ; ↑Σₛ′ = ↑Σₛ′₁ ; ↑Σ≼ₕ = Σ≼ₕ′ ; ↑Σ≼ₛ = Σ≼ₛ′
             ; ↑⊢v = TVClos {𝓢 = 𝓢ᶜ} {x = X s _} {S₁≤x = S₁≤x} ∀⊨𝓔 refl qbd ⊢e₂ wf₁ wf₂ (SQual qsub (SFun{S₁ = S₁@ (T₁ ^ 𝟙)} S₁<⦂S₂ S₃<⦂S₄))
             ; ↑⊢𝓗 = ↑⊢𝓗₁ ; ↑⊢𝓢 = ↑⊢𝓢₁ ; ↑𝓢≼ = 𝓢≼′ }
  with eval-soundness (↑⊢𝓗 ih) (↑⊢𝓢 ih)
                      (⊨-extend (↑Σ≼ₕ ih) (↑Σ≼ₛ ih) (↑𝓢≼ ih) ⊨𝓔)
                      (wfe-ext-≼ₛ (↑𝓢≼ ih) 𝓔-wft)
...
... | ih₂
    = record
        ...
        ; ↑Σₛ′ = ↑Σₛ′ ih₂
        ; ↑Σ≼ₕ = ≼-trans Σ≼ₕ′ (≼-trans Σ≼ₕ″ (↑Σ≼ₕ ih₂))
        ; ↑Σ≼ₛ = ≼-trans Σ≼ₛ′ (≼-trans Σ≼ₛ″ (↑Σ≼ₛ ih₂))
        ; ↑⊢v = <⦂-val-lift (↑⊢v ih₂) S₃<⦂S₄
        ; ↑⊢𝓗 = ↑⊢𝓗 ih₂
        ; ↑⊢𝓢 = ↑⊢𝓢 ih₂
        ; ↑𝓢≼ = ≼ₛ-trans {𝓢₁ = 𝓢} {𝓢₂ = 𝓢′} {𝓢₃ = 𝓢‴} 𝓢≼′ (≼ₛ-trans {𝓢₁ = 𝓢′} {𝓢₂ = 𝓢″} {𝓢₃ = 𝓢‴} 𝓢≼″ (↑𝓢≼ ih₂))
        ; ↑cls = ↑cls ih₂
        ; ↑wf-𝓗 = ↑wf-𝓗 ih₂
        ; ↑wf-𝓢 = ↑wf-𝓢 ih₂
        }
...
eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {q = 𝟙} {q₁ = 𝟚} q₂≤q ⇓ ⇓₁ ⇓₂ 𝓢′≡) = {!!}

-- application, case 4

eval-soundness ⊢𝓗 ⊢𝓢 ⊨𝓔 𝓔-wft wf-𝓗 wf-𝓢 ≤-q (TApp ⊢e ⊢e₁) (EApp {q = 𝟚} {q₁ = 𝟚} q₂≤q ⇓ ⇓₁ ⇓₂ 𝓢′≡) = {!!}
...
... | ih
 ...
 = record
     ...
     ; ↑cls = WFV (λ ())
     ; ↑wf-𝓗 = wfl-ext-≼ₛ (≼ₛ-salloc{𝓢 = 𝓢′}) (↑wf-𝓗 ih)
...
... | ih₁
  ...
  = record
              ...
              ; ↑𝓢≼ = ≼ₛ-trans{𝓢₁ = 𝓢}{𝓢₂ = 𝓢′}{𝓢₃ = 𝓢″} (ih .↑𝓢≼) (ih₁ .↑𝓢≼)
              ; ↑cls = WFV (λ())
              ; ↑wf-𝓗 = wfl-write (ih₁ .↑cls) hwrite (ih₁ .↑wf-𝓗)
              ; ↑wf-𝓢 = ih₁ .↑wf-𝓢
              }
